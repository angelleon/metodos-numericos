...
def print_resultados(p: funcion.Polinomio, N: int, n_files: int):
    print("\n=========================================================================================================")
    print()
    print("Se encontró el polinomio de regresión:")
...
def minimos_cuadrados(x_vect: list, y_vect: list, n: int) -> funcion.Polinomio:
    ...
    for key in sorted(sumas.keys()):
        log.debug(str(key) + ": " + str(sumas[key]))
    m_aum.gauss_jordan()
    log.debug(repr(m_aum))
...
def print_puntos(x_vect: list, y_vect: list):
    print("\nPuntos leidos:")
    for x_i, y_i in zip(x_vect, y_vect):
        print("({}, {})".format(x_i, y_i))
...
def main(argv):
    ...
    for x_i, y_i in zip(x_vect, y_vect):
        ...
        cont += 1
    print_puntos(x_vect, y_vect)
    N = len(x_vect)
    n = leer_entero("el grado del polinomio", "n")
    p = minimos_cuadrados(x_vect, y_vect, n)
    suma_cuad = sum_cuadrados(p, x_vect, y_vect)
    log.debug(suma_cuad)
